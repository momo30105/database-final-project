...
switch ($action) {
   case "update": // 更新
	  if (isset($_POST["Yes"])) {  //若按的是確定更新按鈕
		...
		}else header("Location: search_cus.php"); // 轉址serach.php
		
		//mysqli_query($db, $sql); // 執行SQL指令
	  }
	  else if (isset($_POST["No"])) {
	  header("Location: search_cus.php"); // 轉址serach.php 
      break;
	  }
   case "edit":   // 編輯
...
?>
<h2>編輯客戶基本資料</h2><hr/>
<form action="edit_cus.php?action=update&id=<?php echo $id ?>"
